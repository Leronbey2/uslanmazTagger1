...
async def handler(event):
    ...
    if str(event.sender_id) not in SUDO_USERS:
        return await event.reply("__Sen sahibim değilsin Adamim !__")
    await event.reply('**Hey Bot Çalışıyor Merak Etme Her Zaman Yanindayim 💋💖** \n Sahibim @Serserizarog')

@client.on(events.NewMessage(pattern='/durum'))
async def handler(event):
	
    await event.reply('**Tagger Botun Durum Menüsü** \n Durum: Çalışıyor✅ \n Telethon Sürümü: v1.24.0 \n Python Sürümü: v3.8+ \n Bot Sürümü: v0.2 \n  Botun Developeri @Serserizarog dur')
...
print(">> Bot çalıyor merak etme 🚀 @Serserizarog 'Dan bilgi alabilirsin <<")
